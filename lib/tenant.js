            ...
            function(asyncCallback) {
                ...
                async.forEach(permissions, function (permissionDescriptor, foreachCallback) {
                    ...
                    for(var i = 0; i < actions.length; i++) {
                        if(resourceActionsMap[actions[i]] == null) {
                            continue;
                        }
                        for(var j = 0; j < roles.length; j++) {
                            if(rolesMap[roles[j]] == null) {
                                continue;
                            }
                            permissionDetailsArr.push({
                                aclRole: rolesMap[roles[j]],
                                aclResource: resource,
                                actionName: resourceActionsMap[actions[i]]
                            });
                        }
                    }
