...
class MultiTenant extends events.EventEmitter {
    ...
    /**
     * Initialize tenant from request and return tenant instance
     *
     * @param request
     */
    tenant (request) {
        ...
        if (this._tenants[tenantId] != null) {
            ...
            this._logger.error('@@ Failed to find tenant %s', tenantIdString);
    ...
    addTenant (tenantConfig, callback) {
        ...
        if (!tenantConfig.isEnabled) {
            this._logger.log('## Tenant is not enabled: "%s". SKIPPING ', tenantConfig.name);

            return callback(new Error('Tenant is not enabled: ' + tenantConfig.name, 1038));
        }

        // Safely set mongodbUrl for the tenant
        if (this._config != null) {
            tenantConfig.mongodbUrl = 'mongodb://' + this._config.MONGODB_TENANTS_HOST + ':' + this._config.MONGODB_TENANTS_PORT + '/' + 'tenant_' + tenantConfig.tenantId;
        }
         ...
         * @type {*}
         */
        this._tenants[tenantConfig.tenantId] = new Tenant(tenantConfig);
     ...
     * @param hostname
     */
    loadAdminTenant (hostname, callback) {
        var $this = this;

        /**
         * Proceed with async tenant loading logic
         *
         * @type {{hostname: *}}
         */
        var locals = {hostname: hostname};
        async.series([
            function (asyncCallback) {
                if ($this._hostnameLocator[hostname] != null && $this._adminTenants[$this._hostnameLocator[hostname]] != null) {
                    locals.tenantId = $this._hostnameLocator[hostname];
                    asyncCallback();
                } else {
                    $this._loader.loadTenantByHost(hostname, function(error, tenantDetails) {
                        if (tenantDetails != null) {
                            $this._hostnameLocator[hostname] = tenantDetails.tenantId;
                            locals.tenantId = tenantDetails.tenantId;
                            locals.tenantDetails = tenantDetails;

                            locals.tenantDetails.mongodbUrl = 'mongodb://' + this._config.MONGODB_TENANTS_HOST
                                + ':' + this._config.MONGODB_TENANTS_PORT + '/' + 'tenant_' + locals.tenantDetails.tenantId;

                            /**
                             * Creating Tenant Instance
                             * @type {*}
                             */
                            $this._adminTenants[locals.tenantDetails.tenantId] = new Tenant(locals.tenantDetails);
                            $this._adminTenants[locals.tenantDetails.tenantId].setTemplatesLoader(this._templateLoader);
                            $this._adminTenants[locals.tenantDetails.tenantId].init(function (){
                                // Initialization finished
                                asyncCallback();
                            });
                        } else {
                            asyncCallback(new Error('Specified client is not found'));
                        }
                    });
                }
            }
        ], function(error) {
            callback(error, $this._adminTenants[locals.tenantId]);
        });
    }
