 ...
 * Multitenant event
 *
 * @type {{TENANT_ADDED: string, TENANT_INITIALIZED: string}}
 */
var MultiTenantEvents = {
    TENANT_ADDED: 'TENANT_ADDED',
    TENANT_INITIALIZED: 'TENANT_INITIALIZED'
}
...
class MultiTenant extends events.EventEmitter {
     ...
     * @param tenantId
     */
    loadTenant(tenantId, callback) {
        // Getting tenant
        if (this._tenants[tenantId] != null) {
            this._logger.log("## Tenant %s found in cache, reusing", tenantId);
            return callback(null, this._tenants[tenantId]);
        } else {
            this._loader.loadTenant(tenantId, (error, tenantDetails) => {
                if (error != null) {
                    return callback(new Error('TenantError: Failed to load tenant details: ', error.message));
                }

                if (tenantDetails != null) {
                    if (this._tenants[tenantDetails.tenantId] != null) {
                        this._logger.log("## Tenant %s found in cache, reusing", tenantDetails.tenantId);
                        return callback(null, this._tenants[tenantDetails.tenantId]);
                    } else {
                        this.addTenant(tenantDetails, (error, tenantInstance) => {
                            callback(error, tenantInstance);
                        });
                    }
                } else {
                    this._logger.error('@@ Failed to find tenant %s', tenantId);
                    return callback(new Error('Customer is not available. Please contact support.'));
                }
            });
        }
    }
    ...
    addTenant (tenantConfig, callback) {
        ...
        this._tenants[tenantConfig.tenantId].init((error) => {
            // Initialization finished
            callback(null, this._tenants[tenantConfig.tenantId]);

            /**
             * Emiting event when initializing new tenant
             */
            this.emit(MultiTenantEvents.TENANT_INITIALIZED, {tenant: this._tenants[tenantConfig.tenantId]});
        });
    ...
    updateTenant (tenantConfig, callback) {
        async.series([
            ...
            function (asyncCallback) {
                this.addTenant(tenantConfig, (error) => {
                    asyncCallback(error);
                });
            }.bind(this)
        ], function (error) {
